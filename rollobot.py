...
class InstagramBot:
	...
	def __init__(self, username, password):
		...
		self.driver = webdriver.Chrome("C:/Users/freir/OneDrive/Documentos/Python/community-manager/chromedriver.exe")
	...
	def login(self):
		...
		login_enter.click()
		with open('followed' + self.username.replace('.', '+'), 'r+') as file:
			if not file.read(1):
	...
	def updateModel(self):
		global pipeline
		global rude
		followed = pd.read_csv('followed' + self.username.replace('.', '+')).set_index('username')
		fw = []
		with open('followers' + rollocriollo.username.replace('.', '+'), 'r') as file:
			for line in file:
		...
		if len(fw) != followers_mine:
			...
			#		else:
			#			continue
			with open('followers' + rollocriollo.username.replace('.', '+'), 'w') as file:
				for name in fw:
					file.write(name + '\n')
...
while total_liked + total_followed < 360:
	...
	if (following / followers) > 0.6 and posts > 2 and private == 0:
		...
		with open('followed' + rollocriollo.username.replace('.', '+'), 'a') as file:
